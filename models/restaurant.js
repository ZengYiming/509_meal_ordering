 ...
 * @param cb 回调
 */
Restaurant.prototype.count = function(opt, cb) {
    var sql = "SELECT COUNT(*) AS count FROM restaurant where 1=1 "+opt.where;
    mysql.query(sql, function(err, rs) {
        if(err) return cb(err);
        if(!rs.length) return cb(err);
        cb(err, rs[0]);
 ...
 * @param cb 回调
 */
Restaurant.prototype.create = function(body, cb) {
    var opt = {
        table: 'restaurant',
        fields: body
    };
 ...
 * @param cb 回调
 */
Restaurant.prototype.update = function(body, cb) {
    var opt = {
        table: 'restaurant',
        fields: body
    };
